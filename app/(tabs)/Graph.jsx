...
const Graph = () => {
  ...
  const { weekData, monthData, yearData } = useSelector((state) => state.diary);
  ...
  const { labels, calorieData, fatData, carbsData, proteinData } = useMemo(() => {
    return selectedRange === "weekly"
  ...
  const barChartData = useMemo(() => ({
    ...
    datasets: [
      {
        data: calorieData.map(value => Math.max(value, 0))
      }
    ]
  }), [labels, calorieData]);
  ...
  }), [labels, fatData, carbsData, proteinData]);
